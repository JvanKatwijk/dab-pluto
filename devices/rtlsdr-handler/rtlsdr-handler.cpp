...
float	offset;
	...
	std::complex<float> localBuf [len];
	for (int i = 0; i < (int)(len / 2); i ++)
            localBuf [i] = std::complex<float>
                                      (convTable [buf [2 * i]],
                                       convTable [buf [2 * i + 1]]);
	if (theStick ->theProcessor -> addSymbol (localBuf, len / 2, &offset)) {
	   if ((offset < -5) || (offset > 5)) {
 	      fprintf (stderr, "offset = %f\n", offset);
	      int frequency    =
	            theStick -> rtlsdr_get_center_freq (theStick -> device);
              (void)(theStick -> rtlsdr_set_center_freq (theStick -> device,
	                                                 frequency + offset));
	   }
	}
}
...
//	Our wrapper is a simple classs
	rtlsdrHandler::rtlsdrHandler (dabProcessor *theProcessor,
	                              int32_t	frequency,
...
void	rtlsdrHandler::setOffset	(int offset) {
