...
int	airspyHandler::Samples	() {
	return _I_Buffer. GetRingBufferReadAvailable ();
}

int	airspyHandler::getSamples	(std::complex<float> *v, int amount) {
...
void	airspyHandler::adjustFreq	(int offset) {
	if ((-4 <= offset) && (offset <= 4))
	   return;
	this	-> vfoFrequency += offset;
	fprintf (stderr, "offset = %d\n", offset);
	int result = airspy_set_freq (device, this -> vfoFrequency);
	if (result != AIRSPY_SUCCESS) {
	   printf ("airspy_set_freq() failed: %s (%d)\n",
	            airspy_error_name((airspy_error)result), result);
	}
