 ...
 *
 *    This file is part of dab-pluto
 *
...
#include	"dab-processor.h"
	...
	sdrplayHandler::sdrplayHandler  (dabProcessor	*mr,
	                                 int32_t	frequency,
	                                 int16_t	ppm,
	                                 int16_t	GRdB,
	                                 int16_t	lnaState,
...
float	offset;
	...
	if (p -> theProcessor -> addSymbol (localBuf, numSamples, &offset))
...
void	sdrplayHandler::setOffset	(int offset) {
mir_sdr_ErrT    err;
int	newFrequency	= this -> vfoFrequency + offset;
        err = mir_sdr_SetRf (double (newFrequency), 1, 0);
	fprintf (stderr, "offset = %d\n", offset);
	if (err != mir_sdr_Success) {
           fprintf (stderr, "Error %d on SetRf (%d)\n", err, newFrequency);
        }
	else
	   this -> vfoFrequency = newFrequency;
}
