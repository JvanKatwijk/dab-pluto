...
static int blocksperCIF [] = {18, 72, 0, 36};
	...
	T_u				= params. get_T_u ();
	nrCarriers			= params. get_carriers ();
	theData				= new std::complex<float> *[params. get_L () + 1];
	for (int i = 0; i < params. get_L (); i ++)
	   theData [i] = new std::complex<float> [T_u];

//	cifVector. resize (55296);
	cifCount		= 0;	// msc blocks in CIF
	theBackends. push_back (new virtualBackend (0, 0));
	BitsperBlock		= 2 * nrCarriers;
...
void    mscHandler::process_mscBlock (std::complex<float> *b, int16_t blkno) {
	...
	if (!running. load ())
	   return;
	memcpy (theData [blkno], b, T_u * sizeof (std::complex<float>));
	usedSlots. Release ();
}
...
//	we make the bits into softbits in the range -127 .. 127
	         ...
	         ibits [nrCarriers + i] =  - imag (r1) / ab1 * 127.0;
	      }
